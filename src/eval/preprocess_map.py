...
def process_and_save_features(file_paths, sd_size, dino_size, layer, facet, model, aug, extractor_vit, num_ensemble,
                              flip=False, do_sd=False, do_dino=False, add_str=''):
    ii = 0
    for file_path in tqdm(file_paths, desc="Processing images (Flip: {})".format(flip)):

        subdir_name = f'Features/{dino_size}'
        output_subdir = file_path.replace('JPEGImages', subdir_name).rsplit('/', 1)[0]
        os.makedirs(output_subdir, exist_ok=True)

        suffix = f'{add_str}_flip' if flip else f'{add_str}'
        output_path = os.path.join(output_subdir, os.path.splitext(os.path.basename(file_path))[0] + f'{suffix}.pt')
        ...
        img1_dino = resize(img1, dino_size, resize=True, to_pil=True)
        ...
        if do_sd:
            ...
            feature_payload['sd'] = accumulated_features

        if do_dino:
            img1_batch = extractor_vit.preprocess_pil(img1_dino)
            with torch.no_grad():
                img1_desc_dino = extractor_vit.extract_descriptors(img1_batch.cuda(), layer, facet)

            feature_payload['dino'] = img1_desc_dino

        if feature_payload:
            try:
                torch.save(feature_payload, output_path)
            except Exception as e:
                print(f"Error saving features of {file_path}: {e}")


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description="Process and save features from images.")
    parser.add_argument('--base_dir', type=str, default='data/SPair-71k/JPEGImages',
    ...
    all_files = sorted(
        [os.path.join(subdir, file) for subdir, dirs, files in os.walk(args.base_dir) for file in files if
    ...
    if args.sd:
        ...
        model, aug = load_model(diffusion_ver='v1-5', image_size=args.sd_size, num_timesteps=50,
    ...
    try:
        process_and_save_features(
            all_files, args.sd_size, args.dino_size, args.layer, args.facet,
            model, aug, extractor_vit, args.num_ensemble,
            flip=args.do_flip, do_dino=args.dino, do_sd=args.sd, add_str=''
        )
