...
def load_features_and_masks(aggre_net, img_path, flip, ensemble, num_patches, device,img_dir,feature_dir,only_dino=False,sd_path_suffix='_sd', dino_path_suffix='_dino'):
    ...
    feature_base = img_path.replace('JPEGImages', 'Features/840').replace(f".{f_img_ending}", '')
    mask_base = img_path.replace('JPEGImages', 'SAMMasks').replace(f".{f_img_ending}", '')
    suffix_flip = '_flip' if flip else ''
    ensemble_folder = f'features_ensemble{ensemble}' if ensemble > 1 else 'features'
    mask_path = f"{mask_base}_mask{suffix_flip}.png"
    feature_path =  f"{feature_base}.pt"
    feature_dict = torch.load(feature_path, map_location=device)
    desc_dino = feature_dict['dino'].to(device)
    desc_dino = desc_dino.squeeze(0).reshape(-1, num_patches, num_patches, desc_dino.shape[-1]).permute(0, 3, 1, 2)
